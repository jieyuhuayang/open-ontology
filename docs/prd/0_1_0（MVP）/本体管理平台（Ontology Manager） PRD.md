...
用户选择 **"Use existing datasource（使用已有数据源）"** 后，展开数据源选择面板。面板采用三区域布局，用户可从已有数据集选择，也可通过 MySQL 导入或本地文件上传来创建新的数据集。

**整体布局：**

```
┌──────────────────────────────────────────────────────────────────┐
│  选择底层数据源（Select Backing Datasource）                       │
├──────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─ A. 已有数据集 ──────────────────────────────────────────┐    │
│  │  [🔍 搜索数据集...]                                       │    │
│  │  ┌──────────────────────────────────────────────────────┐│    │
│  │  │ 名称          │ 来源   │ 行数  │ 列数 │ 导入时间      ││    │
│  │  │ orders_2024   │ MySQL  │ 12K  │ 15  │ 2024-01-15   ││    │
│  │  │ employees ⚠️  │ MySQL  │ 500  │ 8   │ 2024-01-10   ││    │
│  │  │  In use (已被关联)                                    ││    │
│  │  └──────────────────────────────────────────────────────┘│    │
│  └──────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ── 或者创建新数据集 ──                                           │
│                                                                  │
│  ┌─ B. 从 MySQL 导入 ──────┐  ┌─ C. 上传本地文件 ──────────┐    │
│  │  [+ Import from MySQL]   │  │  [📁 Upload Excel/CSV]     │    │
│  └──────────────────────────┘  └────────────────────────────┘    │
└──────────────────────────────────────────────────────────────────┘
```
...
- 已被其他 Object Type 关联（作为 backing datasource）的 Dataset，显示 **"In use"** 标签，行置灰且不可选择（点击时 Tooltip 提示"该数据集已被 \<ObjectTypeName\> 关联"）
...
点击 **"Import from MySQL"** 按钮后，打开 Chapter 8 定义的 **4 步模态子向导**（详见 §8.3—§8.6）：
...
子向导导入成功并点击 **"使用此 Dataset"** 后：
- 模态框关闭
- 主向导的数据源选择面板自动选中刚导入的 Dataset
- 列结构预览区域自动展示该 Dataset 的列信息
- 自动进入向导下一步（元数据配置）
...
点击 **"Upload Excel/CSV"** 按钮后，打开 **3 步模态子向导**：
...
点击 **"Use this Dataset"** 后行为与 MySQL 导入一致：模态框关闭，主向导自动选中新 Dataset 并进入下一步。
...
**字段类型推断规则（Excel / CSV → Ontology Property Type）**
...
> **注意：** 推断结果仅作为默认值，用户可在 Step F2 的列选择区域手动修改每列的类型。
...
Open Ontology 采用**数据拷贝（Dataset Import）模型**：从外部 MySQL 数据库将数据导入到平台内部，创建一份 Dataset 快照。对象类型的底层数据（backing dataset）指向平台内部的 Dataset，而非实时查询外部数据库。

```
外部 MySQL → [一次性快照导入] → 平台内部 Dataset → 对象类型（Object Type）
```

**与 Palantir Foundry 的设计对齐：** Ontology Manager 是纯粹的 Schema 管理工具，不提供独立的数据源管理页面。MySQL 连接配置和数据导入完全内嵌于**对象类型创建/编辑流程**中，在需要指定 backing datasource 时按需触发。

**MVP 范围：**
- 仅支持 **MySQL** 数据源；后续版本扩展 PostgreSQL、CSV 等
- 仅支持**一次性快照导入**，不做定期同步或增量更新
- **不新增**侧边栏页面，流程完全在对象类型编辑器内完成

---

### 8.2 触发入口

在**对象类型创建向导**或**对象类型编辑页**的 "Backing Datasource（底层数据源）" 区域，用户有两个选项：

```
底层数据源（Backing Datasource）
┌─────────────────────────────────────────────┐
│  [选择已有 Dataset ▼]                        │  ← 从已导入的快照中选择
│                                             │
│  + 从 MySQL 新建导入                         │  ← 触发本章描述的导入向导
└─────────────────────────────────────────────┘
```

点击 **"+ 从 MySQL 新建导入"** 后，以**模态框向导（modal wizard）** 形式展开导入流程，共 4 步。

---

### 8.3 第 1 步：配置 MySQL 连接
...
**导入成功：**
- 展示摘要：已导入行数、列数、耗时
- 模态框底部显示 **"使用此 Dataset"** 按钮

点击 **"使用此 Dataset"** 后：

1. 模态框关闭，返回对象类型编辑页
2. Backing Datasource 区域自动选中刚导入的 Dataset
3. 左侧数据源面板（datasource pane）自动展示该 Dataset 的所有列，供属性映射使用（行为与第 2.2 节中选择已有数据源后一致）
4. 主键列（PK）自动标记为候选主键属性
...
### 8.7 字段类型映射（MySQL → Ontology Property Type）
