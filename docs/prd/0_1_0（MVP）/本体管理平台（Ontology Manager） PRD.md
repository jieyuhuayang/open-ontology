...
1. 为新对象类型添加底层数据集（Add a backing dataset to a new object type）。可以按以下方式操作：
   ...
   1. 然后选择下图所示的 添加底层数据集（Add a backing dataset）按钮，从平台内已有 Dataset 中选择作为底层数据集。如需导入新数据，请前往 **Data Connection** 模块。
      1. 注意：同一个数据集只能用于底层一个对象类型。
   2. 在概览页面的 属性 区域选择 新建（Create new），进入属性编辑器；或在对象类型视图侧边栏的 属性 页面选择 编辑属性映射。

   ![](<images/本体管理平台（Ontology Manager） PRD-ZlfAb80UioOxhAxHssicBc9Xnwh.png>)

2. 为新对象类型添加元数据（Add metadata for a new object type）
...
>
> * 如果目标对象类型的底层数据集（backing dataset）中存在列名与某个被复制属性的名称匹配，则这些被复制的属性会自动映射（mapped）到目标对象类型的底层数据集。
>
...
2. 定义链接资源（link resources）
   ...
   * 连接表数据集关系类型（Join table dataset relationship type）
     ...
     在”多对多”基数关系下，你需要选择一个底层数据集（dataset），其中包含第一个对象类型主键（示例中 `Aircraft`）与第二个对象类型（示例中 `Flight`）之间**所有链接组合**。

     “多对多”基数关系需要底层数据集支撑，这是为了使用户能够对该链接类型进行**编辑或回写（edit or write back）**。
     ...
     5. 选择链接类型底层数据集中的列，并将其分别映射到两侧对象类型的主键。
...
1. 进入链接类型视图的 **Datasets（数据集）** 页面。

2. 点击现有数据集旁的 **Select（选择）** 图标。随后你可以浏览并选择可用的数据集。
...
2. **Key（键）**：通过下拉框更改外键，或在多对多链接类型中更改列映射。
   ...
   * 注意：对于多对多（many-to-many）基数的链接类型，后备数据集中的列必须映射到对象类型的主键（primary key）。如果对象类型主键属性的类型与链接类型后备数据集中被映射列的类型不一致，系统会报错并阻止保存。
