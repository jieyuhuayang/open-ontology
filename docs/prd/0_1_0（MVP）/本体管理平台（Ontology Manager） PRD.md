...
<li>对象类型的复制</li>
<li>本体导入和导出</li>
<li>数据源管理（MySQL 连接配置 + 一次性数据导入）</li>
</ol></td>
</tr>
<tr>
<td>P2（本期不做）</td>
...
在对象类型 **Overview（概览）** 标签页的链接类型图中选择某个链接类型（见下图），会打开链接类型视图（包含 **Overview（概览）** 和 **Datasources（数据源）** 页面）。
...
2. 手动创建。如果你在完成对象类型创建流程之前退出向导，后续可以通过手动创建为新对象类型指定元数据（metadata）、底层数据源（backing datasource）、属性映射（property mappings）以及键（主键 primary key 与标题键 title key）。
...
如果你已有包含该对象类型数据的已有数据源（datasource），可以选择“使用已有数据源”。系统将自动填充对象类型的元数据；同时会将底层数据源的每一列自动映射为一个属性（property），但你可以在属性（**Properties**）步骤中删除不需要的已添加属性。



如果你没有包含该对象类型数据的已有数据源，可以选择在没有已有数据源的情况下继续，并选择一个位置来生成用于权限（permissions）的数据集（dataset）。由于某一类型对象的权限由其底层数据源的位置决定，系统会提示你选择一个位置，用于保存一个空数据集（empty dataset）。

![](<images/本体管理平台（Ontology Manager） PRD-image-6.png>)

* 设置元数据（Object type metadata）
...
在分步向导对话框的第三步，管理员可以自定义该对象类型拥有哪些属性（支持的属性类型列表可参见《Supported property types》）。如果你选择了已有数据源，其列会自动完成映射，但你可以在此步骤中删除它们。
...
* **主键（Primary key）**：用于唯一标识该对象类型每个实例的属性。底层数据源中每一行在该属性上必须拥有不同的值。

  * 主键旁边需要有 toollip 提示用户在分配主键之前务必检查底层数据源中是否存在重复值，主键必须对数据源中的每条记录都唯一。
...
1. 为新对象类型添加底层数据源（Add a backing datasource to a new object type）。可以按以下方式操作：
   ...
   2. 然后选择下图所示的 添加底层数据源（Add a backing datasource）按钮，选择任意可用数据源作为底层数据源。
...
2. **Plural display name（复数展示名称，本期不用开发此功能）**：~~任何访问该类型多个对象的用户看到的名称。~~

3. **Aliases（别名）**：用户搜索这些额外术语时，也能找到该对象类型。

4. **Description（描述）**：面向用户应用中访问该类型对象的用户的说明文本。例如，用户在 Object Explorer 中搜索时，会在搜索结果里看到对象类型的描述。

5. **Groups（分组）**：一个或多个标签，用于对对象类型进行分类。

6. **ID**：对象类型的唯一标识符，主要用于在配置用户应用时引用该类型对象。
...
7. **Icon（图标）**：在对象类型视图侧边栏选择默认图标，可自定义对象类型图标与颜色；当用户在用户应用中查看该类型对象时，会显示此图标与颜色。

8. **Backing datasource（底层数据源）**：作为该类型对象属性值来源的数据源。
...
你可以通过以下任一种方式，将属性映射到底层数据源的列：

1. 将列映射为新属性

   在屏幕左侧的数据源面板（datasource pane）中，你可以看到数据源的所有列。将鼠标悬停在要映射的列上，选择“添加为新属性”按钮，即可创建一个映射到该列的新属性。属性 ID、展示名称与基础类型（base type）将根据列名推断生成。

   ![](<images/本体管理平台（Ontology Manager） PRD-FTS9b9nTyo8WztxlfGmcKForn0d.png>)

2. 将列映射到已有属性

   在屏幕左侧的数据源面板中，悬停在一个未映射列上并选择“添加为已有属性”。如果已存在某个属性，其属性 ID 与该列名匹配，则该列会被映射到该现有属性。
...
4. 将所有未映射列映射为新属性
   ...
   在数据源面板中、数据源名称旁边，你会看到 “将所有未映射列添加为新属性”按钮。点击后，会为数据源中所有未映射列创建属性。属性的 ID、展示名称与基础类型将根据对应列推断生成。
...
1. 必填对象类型字段
   ...
   * Plural display name（复数展示名称，本期不做）
...
2. 在 **数据源（Datasources）** 面板顶部选择 “替换”按钮。随后你可以浏览并选择可用的数据源。
...
Open Ontology 采用**数据拷贝（Dataset Import）模型**：从外部数据库将数据导入到平台内部，在平台内创建一份 Dataset 副本。对象类型的底层数据（backing dataset）来自平台内部的 Dataset，而非直接实时查询 MySQL 等外部数据库。

这一模型与 Palantir Foundry 的摄入（ingest）架构一致：

```
外部 MySQL → [一次性导入] → 平台内部 Dataset → 对象类型（Object Type）
```

**MVP 范围说明：**
- MVP 仅支持 **MySQL** 数据源
- 仅支持**一次性快照导入**，不做定期同步或增量更新
- 视图（Views）导入为可选支持，非必须
- 后续版本将支持 PostgreSQL、CSV/Excel 等其他数据源类型，以及定期同步

**侧边栏导航新增两个入口：**
- **数据源（Data Sources）**：管理外部数据库连接配置
- **数据集（Datasets）**：管理平台内部已导入的数据快照

---

### 8.2 数据源列表页（Data Sources Page）

**入口：** 侧边栏 → "数据源（Data Sources）"

该页面展示所有已配置的外部数据源连接，字段如下：

| 字段 | 说明 |
|------|------|
| 名称 | 数据源的 display name |
| 类型 | 数据库类型，MVP 仅有 MySQL |
| 主机（Host） | 数据库服务器地址 |
| 数据库（Database） | 目标数据库名称 |
| 连接状态 | `Connected`（上次测试成功）/ `Error`（上次测试失败）/ `Untested`（从未测试） |
| 创建时间 | 数据源配置的创建时间戳 |

**页面操作：**
- **添加数据源**：右上角按钮，打开添加数据源侧边面板（见 8.3）
- **编辑**：行内操作，打开编辑面板（字段同添加，密码字段留空表示不修改）
- **删除**：行内操作，若该数据源已有关联 Dataset，删除前需弹窗警告并列出受影响的 Dataset
- **测试连接**：行内操作，立即发起连接测试并刷新状态

---

### 8.3 添加 MySQL 数据源（Add MySQL Data Source）

**触发入口：** 数据源列表页 → "添加数据源" 按钮

以侧边面板（side panel）或模态框（modal）形式展示，包含以下表单字段：

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| 数据源名称 | 文本输入 | 是 | — | 平台内显示名，不需要全局唯一 |
...
**操作按钮：**
- **测试连接（Test Connection）**：不保存配置，仅发起一次连接验证。成功时显示绿色 "Connection successful" 提示；失败时显示红色错误信息（如 "Access denied" 或 "Connection timed out"）
- **保存（Save）**：持久化连接配置，**不自动触发数据导入**
- **取消（Cancel）**：关闭面板，不保存

**业务规则：**
1. **密码安全**：密码在后端使用 AES-256 加密存储，任何 API 响应均不返回密码明文；日志中也不得出现密码
2. **重复允许**：同一 host + database + username 组合允许创建多个命名数据源（支持不同权限账号的场景）
3. **连接测试非必须**：用户可以不测试直接保存，但建议先测试

---

### 8.4 浏览数据源（Browse Data Source）

**触发入口：** 数据源列表页 → 点击某个数据源名称

进入数据源详情页后，系统自动拉取该数据库下可用的表和视图列表。

#### 8.4.1 表与视图列表

左侧面板展示当前数据库的所有：
- **Tables（表）**：标记 Table 图标
- **Views（视图）**（可选支持，MVP 可优先做 Table）：标记 View 图标

搜索框支持按名称过滤。

对于已导入的表，显示 **"已导入（Imported）"** 标签；对于尚未导入的表，显示 **"导入（Import）"** 按钮。
...
| 字段 | 说明 |
|------|------|
| 列名（Column Name） | 数据库列名 |
| 数据类型（Type） | MySQL 原始类型（如 `VARCHAR(255)`、`INT`、`DATETIME`） |
| 是否主键 | 标记 PK 图标 |
| 是否可为 NULL | 显示 NULL / NOT NULL |

#### 8.4.3 数据预览

在表结构面板底部，提供"预览数据（Preview Data）"区域，默认展示前 **50 行**数据，以只读表格形式呈现。

---

### 8.5 导入数据表（Import Table as Dataset）

**触发入口：** 数据源详情页 → 选中某张表 → 点击 "导入（Import）" 按钮

以**三步导入向导（3-step wizard）** 引导用户完成导入：

#### 第 1 步：配置导入选项

| 配置项 | 说明 |
|--------|------|
| Dataset 名称 | 默认填充为表名，用户可修改；为平台内部 Dataset 的 display name |
| 选择导入列 | 列表展示所有列（带复选框），默认全选；用户可取消勾选不需要的列 |
| 数据行预览 | 展示前 10 行数据供确认，不可编辑 |
...
- 源数据源名称 + 表名
- 将导入的列数
- 预估行数（通过 `SELECT COUNT(*)` 查询获取）
- **提示文案**：`此操作将创建一份数据快照，导入后不会自动与 MySQL 保持同步。如需更新数据，请重新导入。`

**"确认导入（Confirm Import）"** 按钮触发后台导入任务。
...
- 导入完成后展示：
  - 成功状态：已导入行数、列数、耗时
  - 失败状态：错误信息 + "重试（Retry）" 按钮
- 完成后提供两个快捷操作：
  - **"创建对象类型（Create Object Type）"**：跳转至对象类型创建页，并预选此 Dataset 为 backing datasource
  - **"返回数据源（Back to Data Source）"**：关闭向导，返回数据源详情页

**业务规则：**
1. **允许重复导入**：同一张表可以多次导入，每次创建新的 Dataset 快照；旧快照不自动删除，由用户手动管理
2. **导入中断处理**：导入过程中若连接断开，任务标记为失败状态，展示错误提示，允许用户重试；不会产生残留的部分数据
3. **元数据保留**：每个导入的 Dataset 保留以下元数据：
   - `source_datasource_rid`：来源数据源的 RID
...
当用户在**对象类型创建/编辑页**选择"底层数据源（Backing Datasource）"时：

1. **Dataset 选择器**：下拉列表中会列出所有平台内部的 Dataset（包括从 MySQL 导入的快照），可按名称搜索
2. **选中 Dataset 后**，系统自动扫描该 Dataset 的所有列，并在属性（Properties）区域展示映射建议
3. **"一键导入所有未映射列（Add all unmapped columns as new properties）"** 按钮：将 Dataset 中所有尚未映射的列批量创建为新属性，属性的 API Name 和 Display Name 默认取自列名
4. **主键自动识别**：源表中标记为主键（PK）的列，在属性映射时自动标记为**候选主键属性（Primary Key Property）**

**字段类型映射参考（MySQL → Ontology Property Type）：**
...
### 8.7 数据集管理（Datasets Page）

**入口：** 侧边栏 → "数据集（Datasets）"

该页面列出平台内所有内部 Dataset，字段如下：

| 字段 | 说明 |
|------|------|
| 名称 | Dataset 的 display name |
| 来源 | 来源数据源名称 + 表名（如：`prod-mysql / users`），手动创建的标记为"手动" |
| 行数 | 导入时的行数快照（非实时） |
| 列数 | 包含的列数 |
| 引用对象类型数 | 有多少对象类型正在使用此 Dataset 作为 backing datasource |
| 创建时间 | Dataset 的导入/创建时间 |

**行内操作：**
- **查看详情**：展示列结构和元数据（来源、导入时间、行列数等）
- **删除**：
  - 若该 Dataset **没有**对象类型引用：直接删除，确认弹窗提示
  - 若该 Dataset **有**对象类型引用：弹窗警告，列出所有引用该 Dataset 的对象类型名称，用户确认后才可删除；删除后相关对象类型的 backing datasource 引用置为空
