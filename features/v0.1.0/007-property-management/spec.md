...
本特性为全栈特性（后端 + 前端一起开发），实现对象类型属性的创建、查看、编辑、删除和排序，同时管理主键（Primary Key）和标题键（Title Key）的设置。
...
| 决策项 | MVP 范围 | 说明 |
|--------|----------|------|
| backingColumn（数据源列映射） | 不支持 | MVP 仅语义属性，不配置底层数据源，与链接类型保持一致 |
...
| `backingColumn` | string? | 底层数据源列名 | ❌ MVP 不做 |
...
作为 **本体管理员**，
我希望 **能够为对象类型添加、编辑、删除和排序属性，并配置属性的类型、标识和语义约束**，
...
- **AC8**: Properties 标签页以表格展示所有属性，列包含：展示名称、属性 ID、API 名称、基础类型、状态标签、可见性、是否主键、是否标题键
...
- **AC11**: 以下字段**可编辑**：
  ...
  - `isPrimaryKey` / `isTitleKey`（键标记，受 AC15–AC17 约束）
...
- **AC14**: 每个对象类型有且只有**一个主键属性**和**一个标题键属性**；可以为 null（未设置）
- **AC15**（设置主键）: 将属性设为主键时：
...
- **AC16**（设置标题键）: 将属性设为标题键时：
  - 仅合法类型可设为标题键（见领域模型标题键有效类型表）；不合法时操作置灰，tooltip 说明原因
  - 若已有其他属性是标题键，弹出确认提示"将替换现有标题键属性 [name]"，确认后自动清除旧标题键标记
- **AC17**（主键删除保护）: 主键属性不允许被删除；删除操作置灰并提示"请先更换主键后再删除此属性"
  > 标题键无此删除保护限制

### 删除属性

- **AC18**: 删除前展示确认弹窗，显示属性展示名称和类型
- **AC19**: `active` 状态的属性不允许删除；删除按钮置灰并提示"请先将属性状态更改为 deprecated"
- **AC20**: 主键属性不允许删除（AC17 已覆盖，此处强调后端也须校验）
- **AC21**: 删除成功后属性从列表移除

### 属性排序

- **AC22**: 用户可拖拽调整属性的显示顺序；顺序变更写入 `sortOrder` 字段，通过 Working State 暂存，不立即发布

### 复合类型（增量阶段）

- **AC23**（Array）: 选择 Array 类型时，额外显示「元素类型（Item type）」选择器：
  - 仅允许核心和标准类型（11 种）；不允许嵌套 Array
  - 元素类型一经确认同样不可修改
- **AC24**（Struct）: 选择 Struct 类型时，额外显示字段列表编辑器（Field schema editor）：
  - 每个字段需填写：字段名、字段类型（仅限核心和标准类型，不可为 Array）
  - Struct 不支持嵌套；字段不可为 Array 类型

### Working State 集成

- **AC25**: 所有写入操作（创建 / 编辑 / 删除 / 排序）通过 WorkingStateService 写入草稿，不直接修改 `properties` 主表
- **AC26**: 属性列表查询返回**合并视图**（已发布数据 + 草稿变更）
- **AC27**: 合并视图中每条属性标注变更状态标签：
  - `published` — 无未保存变更
  - `created` — 新建但未发布到本体
  - `modified` — 已修改但未发布
  - `deleted` — 已标记删除但未发布
- **AC28**: 点击顶部 **Save** 后，属性变更随同其他资源变更一起提交到本体
...
| backingColumn 未配置时发布 | 允许；MVP 语义模式不要求数据源映射 |
...
- **一致性**: 后端独立校验所有验证规则（AC4–AC6、AC13–AC17），不依赖前端校验
