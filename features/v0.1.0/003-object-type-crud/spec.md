...
**AC-W3** 向导每一步的"下一步"按钮需满足当前步骤的最低要求后才可点击（如 Step 1 至少选中一个 Dataset 或选择 Continue without datasource；Step 2 至少填写 Display name）；不满足时按钮置灰并显示 Tooltip 提示缺少项。
...
**AC-DS1** Step 1 展示数据源选择面板，包含两个区域：A. 已有数据集列表（含搜索过滤）；B. "Continue without datasource" 按钮（允许跳过数据源选择，直接进入 Step 2）。
