...
- [ ] **T001**: 配置 + 依赖
  - 文件:
    - `apps/server/pyproject.toml` — 确认 `python-multipart` 已安装
  - 完成后执行 `uv sync`
  - 依赖: 无

- [ ] **T002**: Alembic 迁移
  - 文件: `apps/server/alembic/versions/0004_add_dataset_tables.py`
  - 内容:
    - 新建 `datasets` 表（rid TEXT PK, name, source_type, source_metadata JSONB, row_count, column_count, status, imported_at, ontology_rid FK, created_by）
    - 新建 `dataset_columns` 表（rid TEXT PK, dataset_rid FK, name, inferred_type, is_nullable, is_primary_key, sort_order, UNIQUE(dataset_rid, name)）
...
- [ ] **T003**: ORM 模型
  ...
  - 内容:
    - 新增 `DatasetModel`, `DatasetColumnModel`
...
- [ ] **T004**: Domain 模型测试
  ...
  - 依赖: 无
  - AC 覆盖: AC-AT2（intended_actions 数据结构）

- [ ] **T005**: Domain 模型实现
  - 文件:
    - `apps/server/app/domain/dataset.py` — Dataset, DatasetColumn, DatasetListItem, DatasetListResponse
...
- [ ] **T008**: DatasetStorage 测试
  - 文件: `apps/server/tests/unit/test_dataset_storage.py`
  - 内容:
    - `_to_domain()` ORM→Domain 转换（字段映射完整性、columns 关联加载）
    - `_to_list_item()` 列表项转换
    - `list_by_ontology()` 过滤 status='ready'、支持 search
    - `get_by_rid()` 含 columns
  - 依赖: T003（ORM 模型）、T005（Domain 模型）
  - AC 覆盖: AC-DS2（数据集列表）

- [ ] **T009**: DatasetStorage 实现
...
- [ ] **T010**: DatasetService 测试
  ...
  - 内容:
    ...
    - `list()` 含 in-use 标记
  - 依赖: T009（DatasetStorage）、T005（Domain 模型）
  - AC 覆盖: AC-DS3（in-use 标记）、AC-V3（数据源唯一性）

- [ ] **T011**: DatasetService 实现
