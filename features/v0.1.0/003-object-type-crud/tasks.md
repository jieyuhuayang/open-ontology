...
- [ ] **T016**: ~~Dataset Router~~ (已由 010 覆盖) + openapi.json 重新生成 + 集成测试补充
  - 010 已覆盖部分:
    - ✅ `app/routers/datasets.py` — GET /datasets、GET /datasets/{rid}、GET /datasets/{rid}/preview、DELETE /datasets/{rid}
    - ✅ `app/main.py` — datasets router 已注册
    - ✅ `tests/integration/test_dataset_api.py` — 基本集成测试已有
  - 仍需实现:
    - `apps/server/openapi.json` — 重新生成（反映 T013/T015 对 OT 的修改）
    - `apps/server/tests/integration/test_incomplete_object_type.py` — 不完整创建 + 补全 + 发布完整性校验 + 类型兼容性校验
  - 依赖: T013、T015
  - AC 覆盖: AC-MC5~MC6、AC-V4、AC-V6
