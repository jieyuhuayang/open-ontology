...
本特性聚焦于**变更管理的 UI 层**。Working State 的核心基础设施（服务层、合并视图、发布/丢弃事务逻辑）已移至 [F009 Working State 服务层](../009-working-state/spec.md)。

本特性在 F009 提供的后端能力之上，构建用户可见的变更管理交互界面。
...
### 发布（Save / Publish）
- **AC5**: 用户点击"发布"后，调用 F009 的发布 API，草稿变更被提交为一个新的版本记录
...
### 丢弃变更
- **AC10**: 用户可以丢弃当前草稿中的所有未发布变更，调用 F009 的丢弃 API
